...
In terms of error analysis, the confusion matrix provided reveals several important patterns in this model’s predictions. For example, it performs fairly well on genres such as Drama and Comedy. This indicates that the dataset contains a large number of instances in those genre categories. This boost in performance compared to other genres is likely because of the high amount of training examples, which allows the model to better distinguishing and learn features for those genres. 
The model appears to struggle with less frequency and more ambiguous genres, such as Film-Noir, Fantasy, and Western. This suggests that class imbalance in the dataset played a significant role in the model’s performance.

There is as notable confusion between genres which used similar language in their descriptions. For example, the model appears to frequently misclassify Action, Crime, and Drama among each other. It has similar misclassification issues between the Horror and Thriller genres. The model also appears to have a bias toward genres with more training data, such as Drama. 
...
%\begin{figure*}[t]
  %\includegraphics[width=\linewidth]{Lily/Images/confusion_matrices.png} \hfill
  %\caption {Confusion matrices for the BERT-base-uncased transformer model.}
%\end{figure*}
